...
export default function ChapterList({ book, chapters }: ChapterListProps) {
  ...
  useEffect(() => {
    const handleNextChapter = () => {
      if (currentTrack?.bookId === book.id && currentTrack?.chapterIndex !== undefined) {
        const nextIndex = currentTrack.chapterIndex + 1;
        if (nextIndex < chapters.length) {
          const nextChapter = chapters[nextIndex];
          handleChapterClick(nextChapter, nextIndex);
        }
      }
    };
    ...
    const handlePreviousChapter = () => {
      if (currentTrack?.bookId === book.id && currentTrack?.chapterIndex !== undefined) {
        ...
        if (prevIndex >= 0) {
          ...
          handleChapterClick(prevChapter, prevIndex);
        }
      }
    };
